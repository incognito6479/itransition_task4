...
def dailyRevenue(df, data_source):
    ...
    plt.tight_layout()
    plt.savefig(f'./static/img/daily_revenue_{data_source}.png', dpi=1000)
    plt.close()
    return

def format_date(date_str):
    match = re.search(r'(\d+)-(\w+)-(\d{4})', date_str)
    if match:
...
def extractDate(df):
    ...
    df['valid_date'] = np.where(
        df['valid_date'].isna(),
        df['timestamp'].str.replace(r'\w+ (\w+)\s+(\d+) \d{2}:\d{2}:\d{2} (\d{4})',
            lambda m: f"{m.group(2)}-{m.group(1)[:3]}-{m.group(3)}",
            regex=True
        ),
        df['valid_date']
    )
    df['valid_date'] = df['valid_date'].apply(format_date)
    df = df.drop(columns=['timestamp'])
    df = df.rename(columns={'valid_date': 'timestamp'})
    return df
